  ...
  <div class="parent-div row col-12 col-sm-12 col-md-12 col-lg-12">
    ...
    <div v-if="formData.success">
      <b-alert show variant="success">
        ...
        <hr />
        <router-link class="alert-btn" :to="signInPath">Close</router-link>
      </b-alert>
    </div>
    <form @submit="submitData" method="post">
      ...
      <input type="password" name="confirm_password" v-model="formData.confirm_password" />
      <br />
      <p v-if="formData.loading">
        <PulseLoader color="#f67d15db" />
      </p>
      <button v-if="!formData.loading && !formData.success" class="login-btn" type="submit">Reset Password</button>
    </form>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...mapActions(["setUser", "updatePassword"]),
    submitData(e) {
      e.preventDefault();
